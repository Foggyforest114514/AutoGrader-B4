...
async def get_system_stats(
    ...
    avg_result = db.query(sqlfunc.avg(Submission.overall_score)).filter(
    ...
    this_month_users = db.query(User).filter(User.created_at >= this_month_start).count()
    last_month_users = db.query(User).filter(
        User.created_at >= last_month_start, User.created_at < this_month_start
    ).count()

    monthly_changes = {
        "users": this_month_users - last_month_users,
        "courses": 0,
        "submissions": 0,
        "avgScore": 0
    }
    ...
    # 最近提交
    recent = db.query(Submission, User).join(
        User, Submission.student_user_id == User.user_id
    ).filter(
        Submission.status.in_(["COMPLETED", "PENDING"])
    ).order_by(Submission.submitted_at.desc()).limit(5).all()

    recent_list = []
    for sub, usr in recent:
        assignment = db.query(Assignment).filter(
            Assignment.assignment_id == sub.assignment_id
        ).first()
        recent_list.append({
            "id": sub.submission_id,
            "student_name": usr.real_name,
    ...
    # 提交状态分布（饼图数据）
    chart_data = [
        {"label": "已完成", "value": completed_submissions},
        {"label": "待评测", "value": pending_submissions},
        {"label": "运行中", "value": db.query(Submission).filter(Submission.status == "RUNNING").count()},
    ]
    ...
    data["monthlyChanges"] = monthly_changes
    data["recentSubmissions"] = recent_list
