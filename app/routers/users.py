...
from app.schemas import (
    UserInfo, UserUpdate, UserAdminUpdate, StudentCreate, TeacherCreate, ResponseModel
)
...
async def update_user_by_admin(
    ...
    # 更新角色相关扩展表
    if user.role == "student" and user_update.student_id is not None:
        student = db.query(Student).filter(Student.user_id == user_id).first()
        if student:
            student.student_id = user_update.student_id

    if user.role == "teacher":
        teacher = db.query(Teacher).filter(Teacher.user_id == user_id).first()
        if teacher:
            if user_update.teacher_id is not None:
                teacher.teacher_id = user_update.teacher_id
...
@router.post("/student", response_model=ResponseModel)
...
async def reset_user_password(
    user_id: int,
    new_password: str = Query("Pass123456"),
    current_user: User = Depends(require_role("admin")),
    db: Session = Depends(get_db)
):
    """管理员重置任意用户的密码"""
